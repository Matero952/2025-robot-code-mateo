...
public class ClimberSubsystem extends SubsystemBase {
    ...
    private final WPI_TalonSRX lockMotor = new WPI_TalonSRX(ClimberConstants.ROTARY_MOTOR_ID);
    // Bag motor
    ...
    private final RelativeEncoder jointEncoder = climberMotor.getEncoder();
    ...
    // Locking motor is a talon srx
    public void setJoint(double percentage) {
        climberMotor.set(percentage);
    }

    public void setRotary(double percentage) {
        lockMotor.set(percentage);
    }
    ...
    public void stopRotaryMotor(double currentMax) {
        if (lockMotor.getSupplyCurrent() > currentMax) {
            lockMotor.setVoltage(0);
        }
    }
    ...
    public void rotateRotary(double voltage) {
        lockMotor.setVoltage(voltage * currentDirection);
